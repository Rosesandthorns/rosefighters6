...
function charHeight(id: string | null | undefined, form?: string): number {
  ...
  if (id === 'rica') return 70;    // 70 pixels tall as specified
  if (id === 'wisp') return 57;    // 71 - 14 = 57
    ...
    for (const [lobbyId, lobby] of Object.entries(lobbies)) {
            ...
            if (player.characterId === 'rica' && player.activeEffects?.['ricaChargeAttack'] && 
                ...
                if (now % 100 < 20) {
                    for (const targetId of lobbyPlayerIds) {
                        ...
                        const target = players[targetId];
                        if (!target) continue;
                        if (target.x < hbX + hbW && target.x + target.width > hbX &&
                            target.y < hbY + hbH && target.y + target.height > hbY) {
                            const dmg = Math.min(25, target.maxHealth * 0.12);
                            applyDamage(target, dmg, player.id);
                            if (target.characterId !== 'wax') {
                                io.to(target.id).emit('applyKnockback', { vx: player.facing === 'left' ? -15 : 15, vy: -8, stunFrames: 20 });
                            }
                        }
                    }
                }
        ...
        for (const player of Object.values(players)) {
            if (player.id !== proj.ownerId) {
                ...
                const maxY = Math.max(proj.y, prevY) + padY;
  ...
  socket.on('useAbility', (data) => {
      ...
      if (player.characterId === 'mirage') {
          ...
          if (data.ability === 1) {
              ...
              player.cocoRageEnd = Date.now() + duration;
              player.speedMult = (player.speedMult || 0.8) * 8; // 8x speed boost for Coco
                  ...
                  if (dist <= 250) {
                      ...
                      setTimeout(() => {
                          if (players[p.id]) {
                              ...
                              players[p.id].speedMult = char ? char.speedMult : 1.0;
                          }
                      }, duration);
                  }
              });
              ...
              setTimeout(() => { 
                  if (players[player.id]) { 
                      ...
                      delete players[player.id].activeEffects?.['cocoRage'];
          ...
          if (data.ability === 1) {
              ...
              walls[id] = {
                  id, x: player.x + (player.facing === 'right' ? 60 : -80), y: player.y + player.height - 80,
