...
// Find socket.on('useAbility'
const startIdx = code.indexOf("socket.on('useAbility', (data) => {");
if (startIdx !== -1) {
    // Find the NEXT socket.on after it
    let endIdx = code.indexOf("  socket.on(", startIdx + 30);
    if (endIdx === -1) {
        // Just find the end of the file or something. Actually useAbility is usually before playerHit.
        endIdx = code.length;
    }
    
    // We just want to insert `newAbilities` BEFORE the closing `});` of `socket.on('useAbility'`
    // Let's find the last `});` before `endIdx`
    let block = code.substring(startIdx, endIdx);
    let lastBracketIdx = block.lastIndexOf("  });");
    if (lastBracketIdx !== -1) {
        let cleanBlock = block.substring(0, lastBracketIdx);
        // cleanBlock ends with `          }` typically
        // we can just strip any trailing whitespace and add newAbilities
        
        let finalBlock = cleanBlock.replace(/\s+$/, "") + newAbilities + "\n";
        code = code.replace(block, finalBlock + "\n");
    } else {
        console.log("Could not find end of useAbility");
    }
} else {
